...
extern uint64 cas(volatile void *addr , uint64 expected , uint64 newval);

extern int pop(int *head, struct spinlock *lock);
extern void push(int *head,struct proc *p, struct spinlock *list_lock);
...
update_least_used_cpu(int cpu_num)
{
  ...
  } while (cas(&c->process_counter, cur, cur+1));

  struct cpu *c1;
  int i = 0;
  int new_least_used_cpu_num = least_used_cpu_num; 
  for(c1 = cpus; c1 < &cpus[CPUS]; c1++, i++)
  {
    struct cpu *c2 = &cpus[new_least_used_cpu_num];
    if(c1 && (c1->process_counter < c2->process_counter))
      new_least_used_cpu_num = i;
  }
