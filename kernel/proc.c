...
update_least_used_cpu(struct cpu *c)
{
  ...
  } while (cas(&least_used_cpu, cur, cur+1));
  struct cpu *c1;
  int i = 0;
  for(c1 = cpus; c < &cpus[NCPU];c1++, i++)
  {
    if(c1->process_counter < c->process_counter)
    {
      least_used_cpu = i;
      c = c1;
    }
